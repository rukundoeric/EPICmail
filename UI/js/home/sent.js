...
const refleshSentMail = () => {
   try{
	    var sent_body = document.getElementById('sent_mail_list_body');
	    var sent_counter = document.getElementById('sent_mail_list_body');
		var sent = new Sent();
	    for (var i = 0;i<sent.getSentMailList().length; i++) {
	      sent.createRow(sent_body,i,sent.getSentMailList()[i]);
	    }	
      return true;
   }catch(error){
      console.log(error);
